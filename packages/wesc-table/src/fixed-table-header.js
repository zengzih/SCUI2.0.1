export default {
  ...
  render(h) {
    let columnRows = []; // this.columns, this.columns
    let resultCol = [];
    let originColumns = this.store.states.originColumns;
    originColumns = columnRows.filter((item) => {
      return item.visibleBool !== false;
    });
    if (originColumns.length > 1) {
      this.columns.forEach(() => {
        resultCol.push({});
      });
      columnRows = [this.columns, resultCol];
    } else {
      columnRows = [this.columns];
    }
            ...
            <tr>
              {
                this._l(columns, (column, cellIndex) =>
                  <th
                    ...
                    class={[column.id, column.order, column.headerAlign ? 'is-' + column.headerAlign : 'is-center', column.className || '', column.type === 'selection' ? 'el-table-column--selection' : '', !column.children ? 'is-leaf' : '', column.labelClassName]}>
                    <div
                    ...
                    >
                      ...
                      }
                      {
                          column.type !== 'selection' ? column.label : <label class="wesc-checkbox">
                              <input type="checkbox" class={ selectionAll ? 'checked' : 'no-checked' } on-click={($event) => this.handleSelectionAll($event)} />
