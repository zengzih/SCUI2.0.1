  ...
  <span>
    <transition :name="transition" @after-leave="doDestroy">
      <div
        ...
        <div class="popper_close" v-if="isClose" @click="showPopper = false">
          <i class="el-icon-close"></i>
        </div>
        <div class="el-popover__title" v-if="title" v-text="title" :style="getPopoverTitleStyle()"></div>
        <slot>{{ content }}</slot>
      </div>
    </transition>
...
export default {
  ...
  methods: {
    getPopoverTitleStyle() {
      if (this.isClose) {
        return ';width: calc(100% - 40px);float:left;';
      }
    },
    doToggle() {
