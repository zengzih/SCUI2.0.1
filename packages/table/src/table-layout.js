...
class TableLayout {
  ...
  getFitColumnWidth(bodyWidth) {
    ...
    if (flexColumns.length === 1) {
      ...
      const allColumnsWidth = flexColumns.reduce((prev, column) => prev + column.width, 0);
      const flexWidthPerPixel = totalFlexWidth / allColumnsWidth;
      let noneFirstWidth = 0;

      flexColumns.forEach((column, index) => {
        if (index === 0) return;
        const flexWidth = Math.floor(column.width * flexWidthPerPixel);
        noneFirstWidth += flexWidth;
        column.realWidth = column.width + flexWidth;
      });
      flexColumns[0].realWidth = flexColumns[0].width + totalFlexWidth - noneFirstWidth;
    }
  }
  ...
  update() {
    ...
    if (this.store.states.fitColumn) {
      ...
      if (this.store.states.fitColumnAllWidth > bodyWidth - this.gutterWidth) {
        this.scrollX = true;
        this.gutterWidth = 17;
      } else {
        this.scrollX = false;
        this.getFitColumnWidth(bodyWidth);
      }
      if (fixedColumns.length > 0) {
        ...
        // this.fixedWidth = this.store.states.fitColumnAllWidth;
      }
