<template>
  <div class="el-table"
    ...
    <div class="config-column-btn" @click="openConfigColumn" v-if="showConfigTable">
...
<script type="text/babel">
  ...
  export default {
    ...
    methods: {
      ...
      setCurrentRow(row) {
        this.store.commit('setCurrentRow', row, true);
      },
    ...
    created() {
      ...
      this.store.states.fitColumn = this.fitColumn;
      this.debouncedLayout = debounce(50, () => this.doLayout());
      this.store.configColumns = this.configColumns;
    },
    ...
    watch: {
      ...
      autoColMerge: {
        immediate: true,
        handler(val) {
          this.store.commit('setAutoColMerge', val);
        }
      },
      autoRowMerge: {
        immediate: true,
        handler(val) {
          this.store.commit('setAutoRowMerge', val);
        }
      },
      fitColumn(newVal) {
        this.store.states.fitColumn = newVal;
      },
