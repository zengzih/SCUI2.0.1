  ...
  selection: {
    ...
    renderCell: function(h, { row, column, store, $index }) {
      ...
      return <el-checkbox
        ...
        on-input={ () => { store.commit('rowSelectedChanged', row, $index); } } />;
    },
    sortable: false,
    resizable: false
  },
...
const DEFAULT_RENDER_CELL = function(h, { row, column }) {
  ...
  const mergeLabel = column.mergeLabel;
  if (column && column.formatter) {
    return column.formatter(row, column);
  }
  if (property && property.indexOf('.') === -1) {
    if (row[mergeLabel]) {
      return row[mergeLabel];
    } else {
      return row[property];
    }
  }
  ...
  created() {
    ...
    column.renderCell = function(h, data) {
      ...
      /* return _self.showOverflowTooltip || _self.showTooltipWhenOverflow
        ? <div class="cell el-tooltip" style={'width:' + (data.column.colSpan>1? "100%" :((data.column.realWidth || data.column.width) + 'px')) }>{ renderCell(h, data) }</div>
        : <div class="cell">{ renderCell(h, data) }</div>; */
      return _self.showOverflowTooltip || _self.showTooltipWhenOverflow
        ? <div class="cell el-tooltip" style={'width:' + (data.row[data.column.property + 'colspan'] > 1 ? '100%' : ((data.column.realWidth || data.column.width) + 'px')) }>{ renderCell(h, data) }</div>
        : <div class="cell">{ renderCell(h, data) }</div>;
