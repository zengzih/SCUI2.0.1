...
export default {
  ...
  render(h) {
    const columnsHidden = this.columns.map((column, index) => this.isColumnHidden(index));  // 将返回的布尔值存放到columnsHidden中组成一个数组
    return (
      <table
        ...
        cellpadding="0"
        border="0">
        <colgroup> {  /* 生成colgroup 和固定的表头保持一致 */ }
          {
              ...
              [<tr
                ...
                class={ [this.getRowClass(row, $index)] }>
                {
                  this._l(this.columns, (column, cellIndex) =>
                    <td
                      style={this.setTdStyle(row, column, cellIndex)}
                      rowSpan={this.setTdRowSpan(row, column)}
                      colSpan={this.setTdColSpan(row, column)}
                      class={ [column.id, column.align, column.className || '', columnsHidden[cellIndex] ? 'is-hidden' : '', (row[column.property + 'rowspan'] || row[column.property + 'colspan']) > 1 ? 'merge-td' : '' ] }
  ...
  methods: {
    ...
    setTdStyle(row, column, index) {
      ...
      if (column.fixed === 'left') {
        vStyle = ';border-right:1px solid #dfe0ec';
      }
      if (column.fixed === 'right') {
        vStyle = ';border-left:1px solid #dfe0ec';
      }
