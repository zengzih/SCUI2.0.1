  ...
  export default {
    data() {
      return {
        ...
        autoRowMerge: [{fieldName:["id"], startCol: 1, endCol: 2}],
        autoColMerge: [{startCol: 1, endCol: 2, startRow: 1, endRow: 5,mergeLabel:"合并内容"}],
...
单元格行合并
:::demo 通过对`auto-row-merge`属性传入合并对象，设置对指定列单元格的合并，auto-row-merge类型为一个对象数组，其中fieldName为合并行依赖字段,类型为Array/String，若不写则合并所有值相同的单元格，startCol是开始列，endCol是结束列，mergeField为要合并的单元格的字段，非必填。
```html
<template>
<div style="margin: 20px;width:100%">
  <sc-table
    :data="tableDataRow"
    :auto-row-merge="autoRowMerge"
    @header-click="headerClick"
    @selection-change='selectionChange'
    @cell-mouse-enter='cellMouseEnter'
    ref="tableref"
    style="width: 100%">
      <sc-table-column prop="id" label="ID" sortable align="left" ></sc-table-column>
      <sc-table-column prop="name" label="名称" show-overflow-tooltip sortable min-width="120" align="left"></sc-table-column>
      <sc-table-column prop="shopId" label="店铺ID" show-overflow-tooltip sortable min-width="120" align="left"></sc-table-column>
  ></sc-table>
</div>
</template>
<script>export default {
  data() {
    ...
    autoRowMerge: [{fieldName:["id"], startCol: 1, endCol: 3}],
    };
  },
  methods: {
    headerClick(col, event, colindex) {
    },
    merCol(list) {
    },
    selectionChange(sel) {
...
单元格列合并
:::demo 通过对`auto-col-merge`属性传入合并对象，设置对指定行列单元格的合并，auto-col-merge类型为一个对象数组，其中startCol为起始列，endCol为结束列，startRow为起始行，endRow为结束行，mergeLabel为单元格合并后要显示的值，非必填。
```html
<template>
<div style="margin: 20px;width:100%">
  <sc-table
    :data="tableDataCol"
    :auto-col-merge="autoColMerge"
    @header-click="headerClick"
    @selection-change='selectionChange'
    @cell-mouse-enter='cellMouseEnter'
    ref="tableref"
...
<script>export default {
  data() {
    ...
    autoColMerge: [{startCol: 1, endCol: 2, startRow: 1, endRow: 3, mergeLabel:"测试合并"}]
    };
  },
  methods: {
    headerClick(col, event, colindex) {
...
列头加背景颜色
:::demo 通过对`el-table-column`组件添加属性'label-bgcolor'，对当前列标题的设置背景颜色。如：label-bgcolor = "#f0f0f0" 。
```html
 <template>
    <el-table
      :data="tableData"
...
| auto-row-merge | 合并单元格行的参数 | Array | — | — |
| auto-col-merge | 合并单元格列的参数 | Array | — | — |
...
| label-bgcolor | 当前列标题的自定义背景颜色 | string | 所有浏览器支持的颜色格式，如#f0f0f0,red,RGB(255,255,255) | — |
